...
class Found_injections:
    ...
    def Dmid_inter(self,  cte):
        ...
        self.m2_det = self.m1 * (1 + self.z)
        
        Mc = (self.m1_det * self.m2_det)**(3/5) / (self.m1_det + self.m2_det)**(1/5)
        
        return cte * Mc**(5/6)
    
    def Dmid(m1, m2, z, cte):
        """
        Dmid values (distance where Pdet = 0.5) as a function of the masses 
        in the detector frame (our first guess)
    ...
    def sigmoid(dL, dLmid, gamma = -0.18395, delta = 0.1146989, alpha = 2.05, emax = 0.967):
        """
    ...
    def Nexp(self, cte):
        ...
        triple integral of p(dL)*p(m1,m2)*sigmoid( dL, Dmid(m1,m2) )*Ntotal
        ...
        quad_fun = lambda dL_int, y, x: self.Ntotal * self.fun_m_pdf(x, y) *  \
            self.interpolation(dL_int) * self.sigmoid(dL_int, self.Dmid(cte)) 
        
        return integrate.nquad( quad_fun, [[self.mmin, self.mmax], [self.mmin, self.mmax], [0, self.zmax]])[0]
    ...
    def Lambda(self, cte):
        ...
        return self.sigmoid(dL, self.Dmid(cte)) * m_pdf * dL_pdf * self.Ntotal
...
cte_opt, maxL = data.MLE(cte_guess)
