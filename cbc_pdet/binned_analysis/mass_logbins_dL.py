...
for i in range(len(dL)):
    for k in range(len(dL_bin)-1):
        if dL[i]>=dL_bin[k] and dL[i]<dL_bin[k+1]:
            ...
            dLbin_data[k]['far_pfull'].append(far_pfull[i])
            dLbin_data[k]['dL_pdf'].append(dL_pdf[i])


for i in range(NdL):
    dLbin_data[i]['mean_dL_pdf'] = np.average(dLbin_data[i]['dL_pdf'])
    dLbin_data[i]['dL_det'] = len(np.array(dLbin_data[i]['dL'])[np.logical_or(np.logical_or(np.logical_or(np.array(dLbin_data[i]['far_pbbh'])<=1, np.array(dLbin_data[i]['far_gstlal'])<=1), np.array(dLbin_data[i]['far_mbta'])<=1), np.array(dLbin_data[i]['far_pfull'])<=1)])
