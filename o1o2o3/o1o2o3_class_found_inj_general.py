...
class Found_injections:
    ...
    def __init__(self, file, thr_far = 1, thr_snr = 10):
        ...
        #Run
        self.name = file["injections/name"][:]
        self.o1 = self.name == b'o1'
        self.o2 = self.name == b'o2'
        self.o3 = self.name == b'o3'
        ...
        #Joint sampling pdf (probability density function), p(m1,m2)*p(z)*p(s1x,s1y,s1z)*p(s2x,s2y,s2z)
        self.joint_pdf = file["injections/sampling_pdf"][:]
        ...
        #Luminosity distance [Mpc] computed for a flat Lambda-Cold Dark Matter cosmology from the z values
        fun_A = lambda t: np.sqrt(omega_m*(1+t)**3+1-omega_m)
        quad_fun_A = lambda t: 1/fun_A(t)
        
        self.dL = np.array([(c*(1+self.z[i])/H0)*integrate.quad(quad_fun_A, 0, self.z[i])[0] for i in range(len(self.z))])
        
        #pdfs
        self.s1_max = max(max(list(self.s1x), list(self.s1y), list(self.s1z)))
        self.s2_max = max(max(list(self.s2x), list(self.s2y), list(self.s2z)))
        ...
        #         return 0
        #     else:
                # return 1 / (4 * np.pi * smax * (sx**2 + sy**2 + sz**2))
        
        def fun_m_pdf(m1, m2):
            
            mmin = min(min(list(m1), list(m2)))
            mmax = max(max(list(m1), list(m2)))
            alpha = -2.35 ; beta = 1
        ...
        self.m_pdf = fun_m_pdf(self.m1, self.m2)
            
        def p(sx, sy, sz, smax):
            smod = sx**2 + sy**2 + sz**2
            return 1/(4*np.pi*smax*smod)*np.heaviside(smax**2-smod,1)
            
        self.s1_pdf = p(self.s1x, self.s1y, self.s1z, self.s1_max)
        self.s2_pdf = p(self.s2x, self.s2y, self.s2z, self.s2_max)
        
        #joint pdf for mass and distance (z and dL)
        self.z_pdf = self.joint_pdf / (self.m_pdf * self.s1_pdf * self.s2_pdf)
        self.mz_pdf = self.joint_pdf / (self.s1_pdf * self.s2_pdf)
        self.mdL_pdf = self.mz_pdf / dL_dif
        ...
        found_pbbh = self.far_pbbh <= thr_far
        found_gstlal = self.far_gstlal <= thr_far
        found_mbta = self.far_mbta <= thr_far
        found_pfull = self.far_pfull <= thr_far
        found_snr = self.snr >= thr_snr
        
        #indexes of the found injections
        self.found_any = found_pbbh | found_gstlal | found_mbta | found_pfull | found_snr
        print('Total number of found injections: ', self.found_any.sum())     
        
        self.found_o1 = self.found_any | self.o1
        self.found_o2 = self.found_any | self.o2
        self.found_o3 = self.found_any | self.o3
...
file = h5py.File('o1+o2+o3_bbhpop_real+semianalytic-LIGO-T2100377-v2.hdf5', 'r')
...
plt.figure()
plt.scatter(data.dL[data.name==b'o3'], data.mdL_pdf[data.name==b'o3'])
plt.semilogy()
plt.xlabel('dL')
plt.ylabel('p(m)*p(dL)')
plt.title('o3 inj from o1+o2+o3 code')
plt.savefig('pdf_o3_from_o123')

plt.figure()
plt.scatter(data.dL[data.name==b'o1'], data.mdL_pdf[data.name==b'o1'])
plt.semilogy()
plt.xlabel('dL')
plt.ylabel('p(m)*p(dL)')
plt.title('o1 inj from o1+o2+o3 code')
plt.savefig('pdf_o1_from_o123')

plt.figure()
plt.scatter(data.dL[data.name==b'o2'], data.mdL_pdf[data.name==b'o2'])
plt.semilogy()
plt.xlabel('dL')
plt.ylabel('p(m)*p(dL)')
plt.title('o2 inj from o1+o2+o3 code')
plt.savefig('pdf_o2_from_o123')

plt.figure()
plt.scatter(data.dL, data.mdL_pdf)
plt.semilogy()
plt.xlabel('dL')
plt.ylabel('p(m)*p(dL)')
plt.title('o1+o2+o3 inj from o1+o2+o3 code')
plt.savefig('pdf_o123_from_o123')

plt.figure()
plt.scatter(data.z[data.name==b'o3'], data.z_pdf[data.name==b'o3'], s=1)
plt.semilogy()
plt.xlabel('z')
plt.ylabel('p(z)')
plt.title('o3 inj from o1+o2+o3 code')
plt.savefig('z_pdf_o3_from_o123')

plt.figure()
plt.scatter(data.z[data.name==b'o1'], data.z_pdf[data.name==b'o1'], s=1)
plt.semilogy()
plt.xlabel('z')
plt.ylabel('p(z)')
plt.title('o1 inj from o1+o2+o3 code')
plt.savefig('z_pdf_o1_from_o123')

plt.figure()
plt.scatter(data.z[data.name==b'o2'], data.z_pdf[data.name==b'o2'], s=1)
plt.semilogy()
plt.xlabel('z')
plt.ylabel('p(z)')
plt.title('o2 inj from o1+o2+o3 code')
plt.savefig('z_pdf_o2_from_o123')