...
def logL_quad_2(in_param, dL, pdL, Total_expected, gamma, delta):
    dLmid, emax = in_param[0], in_param[1]
...
def MLE_2(dL, pdL, dLmid_guess, emax_guess, Total_expected, gamma_new, delta_new):
    res = opt.minimize(fun=lambda in_param, dL, pdL: -logL_quad_2(in_param, dL, pdL, Total_expected, gamma_new, delta_new), 
                       x0=np.array([dLmid_guess, emax_guess]), 
                       args=(dL, pdL,), 
                       method='Nelder-Mead',
                       bounds=[(0, None), (0,1)])
    
    dLmid_res, emax_res = res.x 
...
try:
    os.mkdir('dL_joint_fit_results_emax_bound')
except OSError as e:
    if e.errno != errno.EEXIST:
        raise

try:
    os.mkdir('dL_joint_fit_results_emax_bound/dLmid')
except OSError as e:
    if e.errno != errno.EEXIST:
        raise
        
try:
    os.mkdir('dL_joint_fit_results_emax_bound/emax')
except OSError as e:
    if e.errno != errno.EEXIST:
        raise

try:
    os.mkdir('dL_joint_fit_results_emax_bound/maxL')
except OSError as e:
    if e.errno != errno.EEXIST:
        raise

try:
    os.mkdir('dL_joint_fit_results_emax_bound/final_plots')
except OSError as e:
    if e.errno != errno.EEXIST:
        raise
...
for k in range(0,10000):
    ...
    name = f"dL_joint_fit_results_emax_bound/dLmid/dLmid_{k}.dat"
    np.savetxt(name, dLmid_inter, fmt='%10.3f')
    
    name = f"dL_joint_fit_results_emax_bound/emax/emax_{k}.dat"
    np.savetxt(name, emax_inter, fmt='%10.3f')
    
    name = f"dL_joint_fit_results_emax_bound/maxL/maxL_{k}.dat"
...
np.savetxt('dL_joint_fit_results_emax_bound/all_delta.dat', np.delete(all_delta, 0), fmt='%10.5f')
np.savetxt('dL_joint_fit_results_emax_bound/all_gamma.dat', np.delete(all_gamma, 0), fmt='%10.5f')
np.savetxt('dL_joint_fit_results_emax_bound/total_lnL.dat', np.delete(total_lnL, 0), fmt='%10.3f')
...
k = 20 #number of the last iteration

dLmid_plot = np.loadtxt(f'dL_joint_fit_results_emax_bound/dLmid/dLmid_{k}.dat')
gamma_plot = np.loadtxt('dL_joint_fit_results_emax_bound/all_gamma.dat')[-1]
delta_plot = np.loadtxt('dL_joint_fit_results_emax_bound/all_delta.dat')[-1]
emax_plot = np.loadtxt(f'dL_joint_fit_results_emax_bound/emax/emax_{k}.dat')
...
for i in range(15):
    for j in range(15):
        ...
        #name=f"dL_joint_fit_results_emax_bound/final_plots/{i}_{j}.png"
        name=f"dL_joint_fit_results_emax_bound/final_plots/{i}_{j}.png"
