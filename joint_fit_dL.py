...
def sigmoid_2(dL, dLmid, gamma, delta, alpha=2.05, emax=0.967):
    denom = 1. + (dL/dLmid) ** alpha * \
        np.exp(gamma * ((dL / dLmid) - 1.) + delta * ((dL**2 / dLmid**2) - 1.))
    return emax / denom


def integrand_2(dL_int, dLmid, gamma, delta, alpha=2.05, emax=0.967):
    return dLpdf_interp(dL_int) * sigmoid_2(dL_int, dLmid, gamma, delta, alpha, emax)


def lam_2(dL, pdL, dLmid, gamma, delta, alpha=2.05, emax=0.967):
...
def logL_quad_2(in_param, dL, pdL, Total_expected, gamma, delta):
    ...
    quad_fun = lambda dL_int: Total_expected * integrand_2(dL_int, dLmid, gamma, delta)
    # it's hard to check here what is the value of new_try_z
    Lambda_2 = integrate.quad(quad_fun, min(new_try_dL), max(new_try_dL))[0]
    lnL = -Lambda_2 + np.sum(np.log(Total_expected * lam_2(dL, pdL, dLmid, gamma, delta)))
    return lnL
...
def logL_quad_2_global(in_param, nbin1, nbin2, dLmid_inter):
    ...
    for i in range(0, nbin1):
        for j in range(0, nbin2):
            ...
            quad_fun = lambda dL_int: Total_expected * integrand_2(dL_int, dLmid_inter[i,j], gamma, delta)
            Lambda_2 = integrate.quad(quad_fun, min(new_try_dL), max(new_try_dL))[0]
            #print(Lambda_2)
            lnL = -Lambda_2 + np.sum(np.log(Total_expected * lam_2(dL, pdL, dLmid_inter[i,j], gamma, delta)))
...
def MLE_2(dL, pdL, dLmid_guess, Total_expected, gamma_new, delta_new):
...
def MLE_2_global(nbin1, nbin2, dLmid_inter, gamma_guess, delta_guess):
...
try:
    os.mkdir('dL_joint_fit_results')
except OSError as e:
    if e.errno != errno.EEXIST:
        raise

try:
    os.mkdir('dL_joint_fit_results/dLmid')
except OSError as e:
    if e.errno != errno.EEXIST:
        raise

try:
    os.mkdir('dL_joint_fit_results/maxL')
except OSError as e:
    if e.errno != errno.EEXIST:
        raise

try:
    os.mkdir('dL_joint_fit_results/final_plots')
except OSError as e:
    if e.errno != errno.EEXIST:
        raise
...
for k in range(0,10000):
    ...
    gamma_new, delta_new, maxL_global = MLE_2_global(nbin1, nbin2, dLmid_inter, gamma_new, delta_new)
    ...
    for i in range(0, nbin1):
        for j in range(0, nbin2):
            ...
            Total_expected = NTOT * mean_mass_pdf[i,j]
            dLmid_new, maxL = MLE_2(dL, pdL, dLmid_inter[i,j], Total_expected, gamma_new, delta_new)
    ...
    name = f"dL_joint_fit_results/dLmid/dLmid_{k}.dat"
    np.savetxt(name, dLmid_inter, fmt='%10.3f')
    
    name = f"dL_joint_fit_results/maxL/maxL_{k}.dat"
...
dLmid_plot = np.loadtxt(f'dL_joint_fit_results/dLmid/dLmid_{k}.dat')
gamma_plot = np.loadtxt('dL_joint_fit_results/all_gamma.dat')[-1]
delta_plot = np.loadtxt('dL_joint_fit_results/all_delta.dat')[-1]
...
for i in range(0,nbin1):
    for j in range(0,nbin2):
        ...
        plt.plot(dL_com_1, sigmoid_2(dL_com_1, dLmid_plot[i,j], gamma_plot, delta_plot), '-', label=r'$\varepsilon_2$')
        ...
        name=f"dL_joint_fit_results/final_plots/{i}{j}.png"
