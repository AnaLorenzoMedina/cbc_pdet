...
def logL_quad_2(in_param, dL, pdL, Total_expected, gamma, delta, emax):
...
def logL_quad_2_global(in_param, nbin1, nbin2, dLmid_inter):
    ...
    gamma, delta, emax = in_param[0], np.exp(in_param[1]), np.exp(in_param[2])
    ...
    for i in range(0, nbin1):
        for j in range(0, nbin2):
            ...
            quad_fun = lambda dL_int: Total_expected * integrand_2(dL_int, dLmid_inter[i,j], gamma, delta, emax)
            Lambda_2 = integrate.quad(quad_fun, min(new_try_dL), max(new_try_dL))[0]
            #print(Lambda_2)
            lnL = -Lambda_2 + np.sum(np.log(Total_expected * lam_2(dL, pdL, dLmid_inter[i,j], gamma, delta, emax)))
...
def MLE_2(dL, pdL, dLmid_guess, Total_expected, gamma_new, delta_new, emax_new):
    res = opt.minimize(fun=lambda in_param, dL, pdL: -logL_quad_2(in_param, dL, pdL, Total_expected, gamma_new, delta_new, emax_new), 
...
def MLE_2_global(nbin1, nbin2, dLmid_inter, gamma_guess, delta_guess, emax_guess):
    res = opt.minimize(fun=lambda in_param: -logL_quad_2_global(in_param, nbin1, nbin2, dLmid_inter), 
                       x0=np.array([gamma_guess, np.log(delta_guess), np.log(emax_guess)]), 
                       args=(), 
                       method='Nelder-Mead')
    
    gamma, logdelta, logemax = res.x 
...
try:
    os.mkdir('dL_joint_fit_results_emax')
except OSError as e:
    if e.errno != errno.EEXIST:
        raise

try:
    os.mkdir('dL_joint_fit_results_emax/dLmid')
except OSError as e:
    if e.errno != errno.EEXIST:
        raise

try:
    os.mkdir('dL_joint_fit_results_emax/maxL')
except OSError as e:
    if e.errno != errno.EEXIST:
        raise

try:
    os.mkdir('dL_joint_fit_results_emax/final_plots')
except OSError as e:
    if e.errno != errno.EEXIST:
        raise
...
all_emax = np.array([emax_new])

for k in range(0,10000):
    print('\n\n')
    print(k)
    
    gamma_new, delta_new, emax_new, maxL_global = MLE_2_global(nbin1, nbin2, dLmid_inter, gamma_new, delta_new, emax_new)
    
    all_delta = np.append(all_delta, delta_new) 
    all_gamma = np.append(all_gamma, gamma_new)
    all_emax = np.append(all_emax, emax_new)
    ...
    for i in range(0, nbin1):
        for j in range(0, nbin2):
            ...
            Total_expected = NTOT * mean_mass_pdf[i,j]
            dLmid_new, maxL = MLE_2(dL, pdL, dLmid_inter[i,j], Total_expected, gamma_new, delta_new, emax_new)
    ...
    name = f"dL_joint_fit_results_emax/dLmid/dLmid_{k}.dat"
    np.savetxt(name, dLmid_inter, fmt='%10.3f')
    
    name = f"dL_joint_fit_results_emax/maxL/maxL_{k}.dat"
...
np.savetxt('dL_joint_fit_results_emax/all_delta.dat', np.delete(all_delta, 0), fmt='%10.5f')
np.savetxt('dL_joint_fit_results_emax/all_gamma.dat', np.delete(all_gamma, 0), fmt='%10.5f')
np.savetxt('dL_joint_fit_results_emax/all_emax.dat', np.delete(all_emax, 0), fmt='%10.5f')
np.savetxt('dL_joint_fit_results_emax/total_lnL.dat', np.delete(total_lnL, 0), fmt='%10.3f')
...
dLmid_plot = np.loadtxt(f'dL_joint_fit_results_emax/dLmid/dLmid_{k}.dat')
gamma_plot = np.loadtxt('dL_joint_fit_results_emax/all_gamma.dat')[-1]
delta_plot = np.loadtxt('dL_joint_fit_results_emax/all_delta.dat')[-1]
emax_plot = np.loadtxt('dL_joint_fit_results_emax/all_emax.dat')[-1]
...
for i in range(0,nbin1):
    for j in range(0,nbin2):
        ...
        plt.plot(dL_com_1, sigmoid_2(dL_com_1, dLmid_plot[i,j], gamma_plot, delta_plot, emax_plot), '-', label=r'$\varepsilon_2$')
        ...
        name=f"dL_joint_fit_results_emax/final_plots/{i}_{j}.png"
